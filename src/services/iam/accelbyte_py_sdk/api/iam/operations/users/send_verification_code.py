...
class SendVerificationCode(Operation):
    """[DEPRECATED] Send verification code to user (SendVerificationCode)
    ...
    **This endpoint is deprecated.**
    ...
    _path: str = "/iam/namespaces/{namespace}/users/{userId}/verificationcode"
    _base_path: str = ""
    _method: str = "POST"
    ...
    class Response(ApiResponse):
        ...
        error_403: Optional[HttpResponse] = None
        error_404: Optional[HttpResponse] = None
        error_409: Optional[HttpResponse] = None
        error_429: Optional[HttpResponse] = None
        error_500: Optional[HttpResponse] = None

        def ok(self) -> SendVerificationCode.Response:
            ...
            if self.error_403 is not None:
                err = ApiError.create_from_http_response(self.error_403)
                exc = err.to_exception()
                if exc is not None:
                    raise exc  # pylint: disable=raising-bad-type
            ...
            if self.error_429 is not None:
                err = ApiError.create_from_http_response(self.error_429)
                exc = err.to_exception()
                if exc is not None:
                    raise exc  # pylint: disable=raising-bad-type
    ...
    def parse_response(self, code: int, content_type: str, content: Any) -> Response:
        ...
        """
        result = SendVerificationCode.Response()
        ...
        if error is not None:
            ...
            if code == 204:
                ...
                result.error_403 = HttpResponse.create(code, content)
                result.error = ApiError.create_from_http_response(result.error_403)
            elif code == 404:
                ...
                result.error_429 = HttpResponse.create(code, content)
                result.error = ApiError.create_from_http_response(result.error_429)
            elif code == 500:
