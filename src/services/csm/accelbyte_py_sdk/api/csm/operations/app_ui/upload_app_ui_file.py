...
class UploadAppUIFile(Operation):
    ...
    _path: str = "/csm/v1/admin/namespaces/{namespace}/app-ui/{appUiName}/files/upload"
    _base_path: str = ""
    _method: str = "POST"
    ...
    class Response(ApiResponse):
        data_200: Optional[ApimodelUploadFileResponse] = None
        ...
        error_502: Optional[ResponseErrorResponse] = None

        def ok(self) -> UploadAppUIFile.Response:
            ...
            if self.error_500 is not None:
                ...
                if exc is not None:
                    raise exc  # pylint: disable=raising-bad-type
            if self.error_502 is not None:
                err = self.error_502.translate_to_api_error()
                exc = err.to_exception()
                if exc is not None:
                    raise exc  # pylint: disable=raising-bad-type
            return self
        ...
        def __iter__(self):
            ...
            elif self.error_502 is not None:
                yield None
                yield self.error_502
            else:
                yield None
                yield self.error
    ...
    def parse_response(self, code: int, content_type: str, content: Any) -> Response:
        ...
        """
        result = UploadAppUIFile.Response()
        ...
        if error is not None:
            ...
            if code == 200:
                result.data_200 = ApimodelUploadFileResponse.create_from_dict(content)
            elif code == 400:
                ...
                result.error_502 = ResponseErrorResponse.create_from_dict(content)
                result.error = result.error_502.translate_to_api_error()
            else:
