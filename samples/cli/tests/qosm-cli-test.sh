...
qosm-update-server-config '{"status": "2t3wo5ju"}' 'zE8rdH48' --login_with_auth "Bearer foo"
qosm-delete-server 'f4NlcxyF' --login_with_auth "Bearer foo"
qosm-set-server-alias '{"alias": "Cq0gNnBU"}' 'sFb5w0F0' --login_with_auth "Bearer foo"
qosm-list-server-per-namespace --login_with_auth "Bearer foo"
qosm-list-server --login_with_auth "Bearer foo"
qosm-heartbeat '{"ip": "BTt9x33R", "port": 25, "region": "yV45WieT"}' --login_with_auth "Bearer foo"
exit()
END
...
$PYTHON -m $MODULE 'qosm-heartbeat' \
    '{"ip": "MZND0YWu", "port": 53, "region": "EoFY8zBy"}' \
    --login_with_auth "Bearer foo" \
    > test.out 2>&1
