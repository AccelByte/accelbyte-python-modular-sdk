...
qosm-update-server-config '{"status": "z3BHRGea"}' 'OvjTKAWs' --login_with_auth "Bearer foo"
qosm-delete-server 'uwvLO8x4' --login_with_auth "Bearer foo"
qosm-set-server-alias '{"alias": "OqMcboUE"}' 'qLQPZ8wJ' --login_with_auth "Bearer foo"
qosm-list-server-per-namespace --login_with_auth "Bearer foo"
qosm-list-server --login_with_auth "Bearer foo"
qosm-heartbeat '{"ip": "1lI3tIAp", "port": 69, "region": "9EaqkwHU"}' --login_with_auth "Bearer foo"
exit()
END
...
$PYTHON -m $MODULE 'qosm-heartbeat' \
    '{"ip": "wyUaFRHt", "port": 93, "region": "Ku8RV564"}' \
    --login_with_auth "Bearer foo" \
    > test.out 2>&1
